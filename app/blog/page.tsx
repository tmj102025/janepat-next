    ...
    <>
      ...
      <section className="gradient-hero">
        <div className="mx-auto max-w-5xl px-6 pt-20 pb-12 md:pt-28">
          <div className="text-[11px] font-mono uppercase tracking-widest text-teal-300">Blog</div>
      ...
      <section className="px-6 py-12">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-wrap gap-2">
            {AI_CATEGORIES.map((c) => (
              <Link
                key={c.slug}
                href={`/ai/${c.slug}`}
                className="rounded-full border border-white/[0.08] bg-[#0f1011] px-4 py-1.5 text-[12px] text-zinc-300 transition hover:border-teal-400/30 hover:text-white"
              >
                {c.name}
              </Link>
            ))}
          ...
          {posts.length === 0 ? (
            <div className="mt-12 rounded-2xl border border-dashed border-white/10 bg-[#0f1011] p-12 text-center">
              <p className="text-[14px] text-zinc-400">บทความใหม่กำลังจะมา ติดตามได้เร็ว ๆ นี้</p>
            </div>
          ) : (
